...
def obtener_usuario(nombre: str):
    """
    Devuelve una tupla con (id, nombre, email, rol, creado) o None.
    Nota: usamos alias para el PK 'userDB404' -> id.
    """
    conn = None
    try:
        conn = get_connection()
        cur = conn.cursor()
        cur.execute(
            "SELECT userDB404 AS id, nombre, email, rol, creado FROM usuarios WHERE nombre = %s",
            (nombre,)
        )
        return cur.fetchone()
    finally:
