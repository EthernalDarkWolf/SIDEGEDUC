...
def home_panel():
    ...
    # Si el rol es Representante, redirigir directamente a la lista de hijos vinculados
    if ctx.get('role_name') and ctx.get('role_name').lower() == 'representante':
    ...
    # Verificar si contraseña es default
    default_password_notification = False
    if ctx['current_user_id']:
        user = Usuarios.query.filter_by(id_user=ctx['current_user_id']).first()
        if user and check_password_hash(user.contrasena, 'sidegeduc.25'):
            default_password_notification = True

    return render_template('home_panel/struct.html', usuario=ctx['user'], developer_priv=ctx['developer_priv'], role_name=ctx.get('role_name'), role_desc=ctx.get('role_desc'), status=ctx.get('status'), roles_count=roles_count, users_by_role=users_by_role, default_password_notification=default_password_notification)
...
def consultas():
    ...
    ctx = get_user_context(session)
    role_name = (ctx.get('role_name') or '').lower()
    if role_name in ['invitado', 'representante', 'profesor', 'empleado']:
        return abort(403)
    try:
...
def administrador_herramientas():
    ...
    user_rows = (
        db.session.query(
            Usuarios,
            Roles.nombre_rol.label('rol_name'),
            StatusUser.estado.label('status_name')
        )
        .join(Roles, Usuarios.id_rol == Roles.id_rol)
        .join(StatusUser, Usuarios.id_status_user == StatusUser.id_status_user)
        .filter(Usuarios.id_user != cur.id_user)
    ...
    users = [
        {
            ...
            'id_status_user': row[0].id_status_user,
    ...
    if request.method == 'POST':
        ...
        reset_password = form.get('reset_password')
        if reset_password:
            target_user = Usuarios.query.filter_by(id_user=int(reset_password)).first()
            if target_user:
                try:
                    from werkzeug.security import generate_password_hash
                    target_user.contrasena = generate_password_hash('sidegeduc.25')
                    db.session.commit()
                    roles_count, users_by_role = compute_role_counts(ctx)
                    return render_template('home_panel/struct.html', usuario=user, developer_priv=developer_priv, role_name=ctx.get('role_name'), role_desc=ctx.get('role_desc'), status=ctx.get('status'), roles_count=roles_count, users_by_role=users_by_role, content_template='home_panel/manage_user.html', users=users, roles=roles, statuses=statuses, message='Contraseña restaurada a default', current_user_id=current_user_id, is_creator=ctx.get('is_creator'))
                except Exception:
                    db.session.rollback()
                    roles_count, users_by_role = compute_role_counts(ctx)
                    return render_template('home_panel/struct.html', usuario=user, developer_priv=developer_priv, role_name=ctx.get('role_name'), role_desc=ctx.get('role_desc'), status=ctx.get('status'), roles_count=roles_count, users_by_role=users_by_role, content_template='home_panel/manage_user.html', users=users, roles=roles, statuses=statuses, message='Error al restaurar contraseña', current_user_id=current_user_id, is_creator=ctx.get('is_creator'))
        elif target:
...
def developer_import_export_vista():
    ...
    if request.method == 'POST':
        ...
        elif action == 'upload_github':
            token = request.form.get('github_token')
            repo = request.form.get('repo')
            if token and repo:
                try:
                    import requests
                    # Primero exportar si no existe backup.sql
                    ...
                    if os.path.exists('backup.sql'):
                        ...
                        # Subir a GitHub
                        import base64
                        url = f"https://api.github.com/repos/{repo}/contents/backup.sql"
                        headers = {
                            'Authorization': f'token {token}',
                            'Accept': 'application/vnd.github.v3+json'
                        }
                        data = {
                            'message': 'Backup de base de datos SIDEGEDUC',
                            'content': base64.b64encode(content.encode('utf-8')).decode('utf-8')
                        }
                        response = requests.put(url, headers=headers, json=data)
                        if response.status_code in [200, 201]:
                            message = 'Backup subido a GitHub correctamente'
                        else:
                            message = f'Error al subir a GitHub: {response.json().get("message", "Error desconocido")}'
                    else:
                        message = 'No se pudo crear el backup'
                except Exception as e:
                    message = f'Error al subir a GitHub: {str(e)}'
            else:
                message = 'Proporciona token y repo de GitHub'
        ...
        try:
            if action == 'change_name':
                ...
                if not photo or photo.filename == '':
                    raise ValueError('Selecciona una imagen para la foto de perfil')
                filename = secure_filename(f"{uid}_{photo.filename}")
                profile_dir = os.path.join(os.getcwd(), 'static', 'uploads', 'profiles')
                os.makedirs(profile_dir, exist_ok=True)
                filepath = os.path.join(profile_dir, filename)
                photo.save(filepath)
                user_obj.foto_perfil = os.path.join('uploads', 'profiles', filename)
                db.session.commit()
